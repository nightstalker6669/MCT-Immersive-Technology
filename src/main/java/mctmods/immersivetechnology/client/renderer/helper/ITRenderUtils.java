...
public class ITRenderUtils {
    ...
    private static final int UV_OFFSET = findOffset(Usage.UV, Type.FLOAT);
    private static final int POSITION_OFFSET = findOffset(Usage.POSITION, Type.FLOAT);
    ...
    private static int findOffset(Usage usage, Type type) {
        int offset = 0;
        for (VertexFormatElement element : ITRenderUtils.FORMAT.getElements()) {
            if (element.usage() == usage && element.type() == type) {
